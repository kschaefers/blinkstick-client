...
namespace HidLibrary
{
    public class HidDevice : IDisposable
    {
		...
		/*
        public byte[] ReadFeatureData(byte reportId)
        {
            if (Capabilities.FeatureReportByteLength <= 0) return new byte[0];
...
*/
        ...
        private static String GetIndexedString(IntPtr hidHandle, int id)
        {
            Boolean success;
            Byte[] Buffer;
            Buffer = new Byte[64 * 2];
            ...
            success = NativeMethods.HidD_GetIndexedString(hidHandle, id, Buffer, Buffer.Length);
            if (success == true)
            {
